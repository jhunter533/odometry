...
double turnOffsetx = 360*(sDist*deltaTheta)/(wheelDiameter*M_PI);
double turnOffsety = 360*(rDist*deltaTheta)/(wheelDiameter*M_PI);
 tOX+=turnOffsetx;
 tOY+=turnOffsety;
...
void driveToP(double xTarget, double yTarget, double targetA) {
 ...
 targetAC=ConvertToRadians(targetA);
  ...
  //while(fabs(yVector)>10 ||fabs(xVector)>10 || fabs(head_Error)>10) {
    while(sqrt(xVector*xVector+yVector*yVector)>1 || (atan2(yVector,xVector))-absoluteAngle >5 ) {
   // driveTo();
   driveToNS();
    turnTo();
    //strafeTo();
