...
double startPosX=(-60*360)/(M_PI*3.25);
double startPosY = (70*360)/(M_PI*3.25);
//double startPosX=(2);
//double startPosY = (2);
...
double absoluteAngle = ConvertToRadians(TurnGyroSmart.rotation(deg));
...
//double turnOffsety = 360*(rDist*deltaTheta*2)/(wheelDiameter);//8 and 1//1 and 6.5
//1 and 3
double arc=ConvertToDeg(deltaTheta)/360*(M_PI*1);
double arc2=ConvertToDeg(deltaTheta)/360*(M_PI*3);
//turnOffsety=(360*3*deltaTheta)/(3.25*M_PI);
//turnOffsetx=(360*1*deltaTheta)/(3.25*M_PI);
turnOffsetx = (arc*360/(M_PI*3.25));
turnOffsety = (arc2*360/(M_PI*3.25));
//turnOffsety = (2*deltaTheta*rDist)/(3.25);
tOY=turnOffsety;
tOX=turnOffsetx;
...
void driveToP(double xTarget, double yTarget, double targetA) {
 ...
 targetAC=ConvertToRadians(targetA);
  ...
  //while(fabs(yVector)>10 ||fabs(xVector)>10 || fabs(head_Error)>10) {
    while(sqrt(xVector*xVector+yVector*yVector)>=10 || ((atan2(yVector,xVector))-absoluteAngle-M_PI/2) >=7 ) {
   // driveTo();
   driveToNS();
    turnTo();
    //strafeTo();

    FrontLeftPower = ((drivePID.powerDrive)/5 +turnPID.powerDrive*8);//+strafePID.powerDrive;
    BackLeftPower = (drivePID.powerDrive/5+turnPID.powerDrive*8);//-strafePID.powerDrive;
    FrontRightPower = (drivePID.powerDrive/5 - turnPID.powerDrive*8);//-strafePID.powerDrive;
  BackRightPower = (drivePID.powerDrive/5 -turnPID.powerDrive*8);//+strafePID.powerDrive;
...
if ((-xTargetLocation<-xPosGlobal-1&&-xTargetLocation<-xPosGlobal+1) || (yTargetLocation<yPosGlobal-1 && yTargetLocation<yPosGlobal+1)){
    ...
    if(sqrt(xVector*xVector+yVector*yVector)<=2 ||(atan2(yVector,xVector)-absoluteAngle-M_PI/2)<=2){
      break;
    }
...
//robot turns to fix x in wrong direction since technically that side gets bigger turning left but we want to turn right
 ...
 printf( "dYG %.5f\n", deltaYGlobal);
 printf( "dxg %.5f\n", deltaXGlobal);
 printf( "atan2 %.5f\n", atan2(yVector,xVector)-absoluteAngle-M_PI/2);
 printf( "dyl %.5f\n", deltaYLocal);
 printf( "dR %.5f\n", deltaR);
 printf( "ds %.5f\n", deltaS);
 printf( "ytarget %.5f\n", yTargetLocation);
